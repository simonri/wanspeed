...
class SDTokenizer:
  ...
  def _try_get_embedding(self, embedding_name: str):
    ...
    embed = load_embed(
      embedding_name, self.embedding_directory, self.embedding_size, self.embedding_key
    )
    if embed is None:
      stripped = embedding_name.strip(",")
      if len(stripped) < len(embedding_name):
        embed = load_embed(
          stripped, self.embedding_directory, self.embedding_size, self.embedding_key
        )
    ...
    for weighted_segment, weight in parsed_weights:
      ...
      for word in to_tokenize:
        ...
        tokens.append(
          [
            (t, weight)
            for t in self.tokenizer(word)["input_ids"][self.tokens_start : end]
          ]
        )
...
class SD1Tokenizer:
  def __init__(
    ...
    setattr(
      self,
      self.clip,
      tokenizer(embedding_directory=embedding_directory, tokenizer_data=tokenizer_data),
    )
