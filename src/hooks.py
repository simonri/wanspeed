...
class WeightHook(Hook):
  ...
  def add_hook_patches(
    ...
    if self.need_weight_init:
      key_map = {}
      if target == EnumWeightTarget.Clip:
        key_map = lora.model_lora_keys_clip(model.model, key_map)
      else:
        key_map = lora.model_lora_keys_unet(model.model, key_map)
      weights = lora.load_lora(self.weights, key_map, log_missing=False)
    else:
      if target == EnumWeightTarget.Clip:
...
class AdditionalModelsHook(Hook):
  ...
  def __init__(self, models: list[any] = None, key: str = None):
...
class TransformerOptionsHook(Hook):
  ...
  def add_hook_patches(
    ...
    registered.add(self)
    patcher_extension.merge_nested_dicts(
      model_options, add_model_options, copy_dict1=False
    )
    return True

  def on_apply_hooks(self, model, transformer_options: dict[str]):
  ...
  def get_hooks_for_clip_schedule(self):
    ...
    for hook in self.get_type(EnumHookType.Weight):
      ...
      for keyframe in hook.hook_keyframe.keyframes:
        if keyframe.start_percent > prev_keyframe.start_percent and not math.isclose(
          keyframe.strength, prev_keyframe.strength
        ):
          hook_schedule.append(
            ((prev_keyframe.start_percent, keyframe.start_percent), prev_keyframe)
          )
...
class HookKeyframeGroup:
  ...
  def prepare_current_keyframe(
    ...
    if self._current_used_steps >= self._current_keyframe.get_effective_guarantee_steps(
      max_sigma
    ):
...
def create_transformer_options_from_hooks(
  model, hooks: HookGroup, transformer_options: dict[str] = None
):
...
def get_patch_weights_from_model(model, discard_model_sampling=True):
  if model is None:
    return None
...
def load_hook_lora_for_models(
  ...
  hook = WeightHook()
  hook_group.add(hook)
  loaded: dict[str] = lora.load_lora(lora, key_map)
  if model is not None:
...
def set_timesteps_for_conditioning(cond, timestep_range: tuple[float, float]):
  if timestep_range is None:
    return cond
  return helpers.conditioning_set_values(
...
def set_default_conds_and_combine(
  ...
  for c, new_c in zip(conds, new_conds):
    ...
    new_c = helpers.conditioning_set_values(new_c, {"default": True})
    # apply timesteps, if present
