...
class CLIPLayer(torch.nn.Module):
  def __init__(
    ...
    self.mlp = CLIPMLP(
      embed_dim, intermediate_size, intermediate_activation, dtype, device, operations
    )
...
class CLIPEmbeddings(torch.nn.Module):
  ...
  def forward(self, input_tokens, dtype=torch.float32):
    return self.token_embedding(input_tokens, out_dtype=dtype) + ops.cast_to(
...
class CLIPTextModel_(torch.nn.Module):
  ...
  def forward(
    ...
    if embeds is not None:
      x = embeds + ops.cast_to(
        self.embeddings.position_embedding.weight, dtype=dtype, device=embeds.device
      )
    else:
