...
class ModelPatcher:
  ...
  def add_weight_wrapper(self, name, function):
    self.weight_wrapper_patches[name] = self.weight_wrapper_patches.get(name, []) + [
      function
    ]
    self.patches_uuid = uuid.uuid4()
        ...
        if lowvram_weight:
          ...
          if bias_key in self.patches:
            if force_patch_weights:
              ...
              m.bias_function = [
                LowVramPatch(bias_key, self.patches, convert_func, set_func)
              ]
              patch_counter += 1
      ...
      if lowvram_counter > 0:
        ...
        logging.info(
          "loaded completely; {:.2f} MB usable, {:.2f} MB loaded, full load: {}".format(
            lowvram_model_memory / (1024 * 1024), mem_counter / (1024 * 1024), full_load
          )
        )
        self.model.model_lowvram = False
        if full_load:
    ...
    with self.use_ejected(skip_and_inject_on_exit_only=True):
      ...
      if extra_memory < 0 and not unpatch_weights:
        self.partially_unload(
          self.offload_device, -extra_memory, force_patch_weights=force_patch_weights
        )
        return 0
      full_load = False
      if (
        self.model.model_lowvram == False and self.model.model_loaded_weight_memory > 0
      ):
        self.apply_hooks(self.forced_hooks, force_apply=True)
        return 0
        ...
        if cached_weights is not None:
          ...
          for key in cached_weights:
            ...
            self.patch_cached_hook_weights(
              cached_weights=cached_weights, key=key, memory_counter=memory_counter
            )
