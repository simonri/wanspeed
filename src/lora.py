    ...
    if k.endswith(".weight"):
      if k.startswith("t5xxl.transformer."):  # OneTrainer SD3 and Flux lora
        ...
        if clip_l_present:
          t5_index += 1
          if t5_index == 2:
            key_map["lora_te{}_{}".format(t5_index, l_key.replace(".", "_"))] = (
              k  # OneTrainer Flux
            )
            t5_index += 1
...
def model_lora_keys_unet(model, key_map={}):
  ...
  if isinstance(model, model_base.SD3):  # Diffusers lora SD3
    diffusers_keys = utils.mmdit_to_diffusers(
  ...
  if isinstance(model, model_base.AuraFlow):  # Diffusers lora AuraFlow
    diffusers_keys = utils.auraflow_to_diffusers(
      model.model_config.unet_config, output_prefix="diffusion_model."
    )
    for k in diffusers_keys:
      if k.endswith(".weight"):
  ...
  if isinstance(model, model_base.PixArt):
    diffusers_keys = utils.pixart_to_diffusers(
      model.model_config.unet_config, output_prefix="diffusion_model."
    )
    for k in diffusers_keys:
      if k.endswith(".weight"):
  ...
  if isinstance(model, model_base.Flux):  # Diffusers lora Flux
    diffusers_keys = utils.flux_to_diffusers(
      model.model_config.unet_config, output_prefix="diffusion_model."
    )
    for k in diffusers_keys:
      if k.endswith(".weight"):
  ...
  for p in patches:
    ...
    if patch_type == "diff":
      ...
      if do_pad_weight and diff.shape != weight.shape:
        logging.info(
          "Pad weight {} from {} to shape: {}".format(key, weight.shape, diff.shape)
        )
      ...
      if strength != 0.0:
        if diff.shape != weight.shape:
          ...
          weight += function(
            strength
            * model_management.cast_to_device(diff, weight.device, weight.dtype)
          )
    elif patch_type == "set":
      weight.copy_(v[0])
    elif patch_type == "model_as_lora":
      target_weight: torch.Tensor = v[0]
      diff_weight = model_management.cast_to_device(
      ...
      weight += function(
        strength
        * model_management.cast_to_device(diff_weight, weight.device, weight.dtype)
      )
    else:
