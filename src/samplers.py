...
from src.cfg_guider import CFGGuider
from src.wan.model_base import BaseModel
...
import src.k_diffusion.sampling as k_diffusion_sampling
...
def get_area_and_mult(conds, x_in, timestep_in):
  ...
  if "mask" in conds:
    ...
    mask = mask.unsqueeze(1).repeat(
      (input_x.shape[0] // mask.shape[0], input_x.shape[1]) + (1,) * (mask.ndim - 1)
    )
  else:
...
def _calc_cond_batch(
  model: BaseModel, conds: list[list[dict]], x_in: torch.Tensor, timestep, model_options
):
  out_conds = []
  out_counts = []
  ...
  for hooks, to_run in hooked_to_run.items():
    while len(to_run) > 0:
      ...
      if "transformer_options" in model_options:
        transformer_options = patcher_extension.merge_nested_dicts(
    ...
    if "area" in x:
      ...
      if area_inside(c_area, a):
        if smallest is None:
          ...
          if math.prod(smallest["area"][: len(smallest["area"]) // 2]) > math.prod(
            a[: len(a) // 2]
          ):
            smallest = x
    else:
...
def preprocess_conds_hooks(conds: dict[str, list[dict[str]]]):
  ...
  hook_replacement: dict[tuple[any, hooks.HookGroup], list[dict]] = {}
  for k in conds:
    for kk in conds[k]:
      if "control" in kk:
        control: any = kk["control"]
