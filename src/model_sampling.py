...
from src.diffusion_utils import make_beta_schedule
...
class ModelSamplingDiscrete(torch.nn.Module):
  ...
  def sigma(self, timestep):
    t = torch.clamp(
      timestep.float().to(self.log_sigmas.device), min=0, max=(len(self.sigmas) - 1)
    )
...
class StableCascadeSampling(ModelSamplingDiscrete):
  ...
  def sigma(self, timestep):
    alpha_cumprod = (
      torch.cos((timestep + self.cosine_s) / (1 + self.cosine_s) * torch.pi * 0.5) ** 2
      / self._init_alpha_cumprod
    )
